...
/-- Bridging lemma to the already-formalized `no_cycle`: a duplicate-free list `L`
of length `≥ 3` whose consecutive entries (cyclically) are `C`-adjacent describes an
injective cycle, and hence cannot exist in a positive-definite Dynkin diagram. -/
lemma no_cycle_list (C : Matrix (Fin n) (Fin n) ℤ) (hGCM : IsGeneralizedCartanMatrix C)
    (L : List (Fin n)) (hnodup : L.Nodup) (hlen : 3 ≤ L.length)
    (hcyc : ∀ i : Fin L.length,
      C (L.get i) (L.get ⟨(i.val + 1) % L.length, Nat.mod_lt _ (by omega)⟩) ≠ 0) :
    ¬ (SymmMatrix C).PosDef := by
  have hemb : Function.Injective L.get := List.nodup_iff_injective_get.mp hnodup
  exact no_cycle C hGCM L.length hlen ⟨L.get, hemb⟩ hcyc
